...
소설의 모든 장을 쓰고 나서는 각 장이 쓰여진 파일을 합쳐서 최종적으로 소설의 완성본이 들어있는 한 개의 파일을 만든다. 이 과정에서 두 개의 파일을 합쳐서 하나의 임시파일을 만들고, 이 임시파일이나 원래의 파일을 계속 두 개씩 합쳐서 파일을 합쳐나가고, 최종적으로는 하나의 파일로 합친다.
...
# 최소 힙을 이용해 파일을 합치는 데 필요한 최소비용을 계산하는 함수
def find_min_merge_cost(file_sizes):
    # 리스트를 최소힙 구조로 변환
    heapify(file_sizes)
    # 전체 비용을 누적하여 계산하는 변수
    total_cost = 0
    ...
    while len(file_sizes) > 1:
        # 가장 작은 크기의 두 파일 a, b 꺼내기(그리디 전략: 항상 작은 것끼리 먼저 합쳐야 전체 비용 최소)
        a = heappop(file_sizes)
        b = heappop(file_sizes)
        # 두 파일의 크기를 합치는 데 필요한 비용은 s
        s = a + b
        # 이번 파일 합치기에서 발생한 비용 누적
        total_cost += s
        # 합쳐진 파일 크기를 다시 힙에 추가
        heappush(file_sizes, s)
    
    # 모든 파일을 하나로 만들 때까지 필요한 최소 비용 반환
    return total_cost

# main
T = int(input())    # T: 테스트 케이스 개수
for _ in range(T):  # 각 테스트 케이스 입력 받기
    K = int(input())    # K: 소설을 구성하는 장의 수
    file_sizes = list(map(int, input().split()))    # 파일 크기 입력 받기
    min_merge_cost = find_min_merge_cost(file_sizes)    # 파일 합치는 데 필요한 최소 비용 계산
    print(min_merge_cost)   # 결과 반환