...
선호도 투표를 통해 얻은 세 후보의 점수를 계산한 후, 유일하게 회장이 결정되는 경우에는 회장으로 결정된 후보의 번호(1, 2, 3 중 한 번호)와 최고 점수를 출력하고, 회장을 결정하지 못하는 경우에는 번호 0과 최고 점수를 출력하는 프로그램을 작성하시오.

[입력]
첫째 줄에는 반의 학생들의 수 N (3 ≤ N ≤ 1,000)이 주어진다.
다음 N개의 각 줄에는 각 학생이 제출한 회장후보 3명에 대한 선호 점수가 주어지는 데, 첫 번째 점수는 후보 1번에 대한 점수이고 두 번째 점수는 후보 2번에 대한 점수이고 세 번째 점수는 후보 3번에 대한 점수이다.
...
각 총점, 3점, 2점 리스트로 저장
투표 입력하며 후보별 점수 & 횟수 누적
총점 가장 큰 후보만 추림
- 여러명일 때 : 3점 횟수가 가장 많은 사람만
-- 같은 수일 때 : 2점 횟수 가장 많은 후보
- 최종 후보 1명일 때 : 번호와 최고 점수
- 최종 여러명 : 0과 최고 점수
'''
n = int(input())
total = [0, 0, 0]
...
if len(result) > 1:
    ...
    for i in result:
        if score_2[i] > max_2:
            max_2 = score_2[i]
            new_result = [i]
        elif score_2[i] == max_2:
            new_result.append(i)
    result = new_result

if len(result) == 1:
    print(result[0] + 1, max_total)
else:
    print(0, max_total)